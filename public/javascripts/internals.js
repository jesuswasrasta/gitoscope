...
$(document).ready(function () {
  ...
  function addCommitToTreeWithInternals(commit) {
    ...
    getTreeRecurse(commit.tree.id).then(() => {
      cy.elements("node[type='tree'], node[type='blob'], edge").layout({
        name: 'dagre',
        rankDir: 'LR',
      });
    });
  ...
  function addCommit(id) {
    const elements = {
      nodes: [
        {
          data: { id: id, name: id.substr(0, 7), type: 'commit' },
          renderedPosition: { x: commitX, y: currentY },
        },
      ],
    };
    if (head) {
      elements.edges = [{ data: { id: id + head, source: id, target: head, weight: 3 } }];
    }
  ...
  function addObject(id, type, relatesTo) {
    ...
    const elements = {
      nodes: [
        { data: { id: id, name: id.substr(0, 7), type: type } },
        { data: { id: relatesTo, name: relatesTo } },
      ],
      edges: [
        {
          data: {
            id: [id, relatesTo].sort().join(),
